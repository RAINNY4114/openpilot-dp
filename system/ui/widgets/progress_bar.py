...
class ProgressBarAction(ItemAction):
  ...
  def update(self, progress: float, text: str, show_progress: bool = False, text_color: rl.Color = rl.GRAY) -> None:
  ...
  def _render(self, rect: rl.Rectangle) -> bool:
    ...
    rl.draw_text_ex(self._font, self.text, rl.Vector2(bar_rect.x + text_x, bar_rect.y + (bar_height - text_size.y) / 2),
                    font_size, 0, self.text_color)
    return False


def progress_item(title: str) -> ListItem:
