  ...
  def __init__(self):
    ...
    self._scroller = Scroller([
      ...
      spin_button_item(
        ...
        description=lambda: tr("Default 100 m (50–150). Highway/low visibility: 110–140; city/low speed: 80–90. Higher = earlier slowdown; lower = waits until closer."),
        initial_value=self._get_param_int("dp_lincoln_curve_window_m", 100),
      ...
      double_spin_button_item(
        title=lambda: tr("Comfort lateral limit"),
        description=lambda: tr("Default 1.20 m/s² (1.0–1.5). Stable ride: 1.0–1.1; faster (use caution): 1.3–1.4. Lower = slower/steadier; higher = faster/near limit."),
        initial_value=self._get_param_int("dp_lincoln_curve_alat", 120) / 100.0,
        callback=lambda val: self._params.put("dp_lincoln_curve_alat", int(round(val * 100))),
        min_val=1.0,
        max_val=1.5,
        step=0.05,
        decimals=2,
      ...
      double_spin_button_item(
        title=lambda: tr("Exit hold time"),
        description=lambda: tr("Default 0.70 s (0.3–1.0). S-bends jitter: 0.8–1.0; few curves: 0.4–0.6. Higher = stay slow longer; lower = release sooner."),
        initial_value=self._get_param_int("dp_lincoln_curve_exit_h", 70) / 100.0,
        callback=lambda val: self._params.put("dp_lincoln_curve_exit_h", int(round(val * 100))),
        min_val=0.3,
        max_val=1.0,
        step=0.1,
        decimals=2,
      ...
      simple_item(title=lambda: tr("### Steering Response ###")),
      spin_button_item(
        title=lambda: tr("Steer rate (turn-in)"),
        description=lambda: tr("Default 100% (80–120), all speeds. Adjust in ±5% steps: higher = quicker turn-in but more warning risk; lower = smoother/slower."),
        initial_value=self._get_param_int("dp_lincoln_steer_rate_up_pct", 100),
        callback=lambda val: self._params.put("dp_lincoln_steer_rate_up_pct", int(val)),
        min_val=80,
        max_val=120,
        step=2,
        suffix=" %",
      ...
      spin_button_item(
        title=lambda: tr("Steer rate (return)"),
        description=lambda: tr("Default 100% (80–120), all speeds. Adjust in ±5% steps: higher = faster recenter but more warning risk; lower = gentler/slower."),
        initial_value=self._get_param_int("dp_lincoln_steer_rate_down_pct", 100),
        callback=lambda val: self._params.put("dp_lincoln_steer_rate_down_pct", int(val)),
        min_val=80,
        max_val=120,
        step=2,
        suffix=" %",
