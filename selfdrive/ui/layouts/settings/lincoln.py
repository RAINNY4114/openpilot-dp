...
class LincolnLayout(Widget):
  def __init__(self):
    ...
    self._scroller = Scroller([
      ...
      simple_item(title=lambda: tr("### SET+/SET- Step Adjustment ###")),
      spin_button_item(
        title=lambda: tr("Short press step (km/h)"),
        description=lambda: tr("Speed change per short press of SET+/SET-."),
        initial_value=max(1, min(5, self._get_param_int("dp_lincoln_set_speed_step_short_kph", 1))),
        callback=lambda val: self._params.put("dp_lincoln_set_speed_step_short_kph", int(val)),
        min_val=1,
        max_val=5,
        step=1,
        suffix=tr(" km/h"),
      ...
      spin_button_item(
        title=lambda: tr("Long press step (km/h)"),
        description=lambda: tr("Speed change per long press of SET+/SET-."),
        initial_value=max(5, min(30, int(round(self._get_param_int("dp_lincoln_set_speed_step_long_kph", 5) / 5)) * 5)),
        callback=lambda val: self._params.put("dp_lincoln_set_speed_step_long_kph", int(val)),
        min_val=5,
        max_val=30,
        step=5,
        suffix=tr(" km/h"),
